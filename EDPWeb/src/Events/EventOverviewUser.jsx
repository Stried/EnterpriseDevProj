...
const truncateText = (text, maxLength) => {
  ...
  >
    Search
        ...
        <Card className="min-w-max w-96 h-80 "  style={{
        boxShadow: hoveredIndex === i ? '10px 10px 20px rgba(0, 0, 0, 0.5)' : '0 4px 6px rgba(0, 0, 0, 0.1)',
        transform: hoveredIndex === i ? 'scale(1.03, 1.03)' : 'scale(1)',
        ...
        <CardContent>
          <h2 className="text-xl font-bold mb-2">{truncateText(event.eventName, 34)}</h2>
          
          <p>Posted by: {event.user.name}</p>
          
         <span className="inline-block"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-map-pin"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"></path><circle cx="12" cy="10" r="3"></circle></svg></span> {truncateText(event.eventLocation, 21)}
          {event.dates.length > 0 && (
                                <p className="ml-7">
           <span className="inline-block"><svg xmlns="http://www.w3.org/2000/svg" style={{ position: 'absolute', top: '105px', left: '16px' }} width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-calendar"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"></rect><line x1="16" y1="2" x2="16" y2="6" ></line><line x1="8" y1="2" x2="8" y2="6"></line><line x1="3" y1="10" x2="21" y2="10"></line></svg></span>                       
    ...
    switch (event.activityType) {
      case 'Dine and Wine':
        return <div className="absolute bottom-0 right-0 border-0 bg-red-400 p-1 rounded-lg text-white">
               <p>{event.activityType}</p>
               </div>;

      case 'Hobbies and Wellness':
        return <div className="absolute bottom-0 right-0 border-0 bg-violet-400 p-1 rounded-lg text-white">
        <p>{event.activityType}</p>
        </div>;
      case 'Family and Friends':
        return <div className="absolute bottom-0 right-0 border-0 bg-sky-400 p-1 rounded-lg text-white">
        <p>{event.activityType}</p>
        </div>;
      case 'Sports':
        return <div className="absolute bottom-0 right-0 border-0 bg-orange-400 p-1 rounded-lg text-white">
        <p>{event.activityType}</p>
        </div>;
      case 'Adventure':
        return <div className="absolute bottom-0 right-0 border-0 bg-yellow-300 p-1 rounded-lg text-white">
        <p>{event.activityType}</p>
        </div>;
      case 'Travel':
        return <div className="absolute bottom-0 right-0 border-0 bg-teal-300 p-1 rounded-lg text-white">
        <p>{event.activityType}</p>
        </div>;
      default:
        return <p>{event.activityType}</p>;
    }
  })()}
{closestDates[i] && (
  <div style={getClosestDateStyles(i)}>
    ...
    <p>
      <span className="inline-block pb-6" >
        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-clock" style={{ position: 'absolute', top: '-15px', left: '235px' }}>
          <circle cx="12" cy="12" r="10"></circle>
          <polyline points="12 6 12 12 16 14"></polyline>
        </svg>
      </span>
      <span className="pl-2">{closestDates[i].toLocaleDateString()}</span>
    </p>
  </div>
)}
            <div
